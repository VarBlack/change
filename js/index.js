	...
	(function(d){
		...
		for(var i=0;i<len;i++){
			...
			for(var j=0, len1=data[i].info.length; j<len1; j++){
				str5+='<img src="img/'+data[i].imgs[j]+'" alt="" />'//生成图片
				str6+='<a href="#">'+data[i].info[j]+'</a>'//生成标签
			}
			str3+='<div class="imgs">'+str5+'</div>';
			str4+='<div class="info">'+str6+'</div>';
			//生成右侧div
//			str2+= '<div class="right">'+str3+''+str4+'/div>';
		}
		str1+='</div>';
		str2+=str3+str4+'</div>';
